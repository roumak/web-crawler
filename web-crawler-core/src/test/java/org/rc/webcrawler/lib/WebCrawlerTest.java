...
class WebCrawlerTest {
    ...
    public void WebPageHandlerTest() {
        ...
        try (MockedStatic<WebPageHandler> mockedStatic = Mockito.mockStatic(WebPageHandler.class)) {
            mockedStatic.when(() -> WebPageHandler.LINK_EXTRACTOR.apply(any(Connection.Response.class), any(Predicate.class)))
                    .thenReturn(Optional.of(urlStream))
                    .thenReturn(Optional.empty());

            Connection.Response mockResponse = Mockito.mock(Connection.Response.class);
            mockedStatic.when(() -> WebPageHandler.FETCHER.apply(anyString(), any(Long.class)))
                    .thenReturn(Optional.ofNullable(mockResponse));

            Assertions.assertEquals(mockResponse, WebPageHandler.FETCHER.apply("/", 100L).get());
            Stream<String> actualUrlSet = WebPageHandler.LINK_EXTRACTOR.apply(Mockito.mock(HttpConnection.Response.class), s -> s.startsWith("/")).get();
